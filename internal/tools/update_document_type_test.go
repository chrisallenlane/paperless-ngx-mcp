...
func TestUpdateDocumentType_Execute(t *testing.T) {
	server := httptest.NewServer(
		http.HandlerFunc(
			func(w http.ResponseWriter, r *http.Request) {
				...
				if patch["name"] != "Updated Type" {
					t.Errorf(
						"name = %v, want Updated Type",
						patch["name"],
					)
				}
	...
	tool := NewUpdateDocumentType(c)

	result, err := tool.Execute(
		context.Background(),
		json.RawMessage(
			`{"id": 1, "name": "Updated Type"}`,
		),
	)
	if err != nil {
...
func TestUpdateDocumentType_Execute_MissingID(t *testing.T) {
...
func TestUpdateDocumentType_Execute_InvalidID(t *testing.T) {
	c := client.New("http://localhost", "test-token")
	tool := NewUpdateDocumentType(c)
...
func TestUpdateDocumentType_Execute_ServerError(t *testing.T) {
	server := httptest.NewServer(
		http.HandlerFunc(
...
func TestUpdateDocumentType_Description(t *testing.T) {
	c := client.New("http://localhost", "test-token")
	tool := NewUpdateDocumentType(c)
...
func TestUpdateDocumentType_InputSchema(t *testing.T) {
	c := client.New("http://localhost", "test-token")
	tool := NewUpdateDocumentType(c)
